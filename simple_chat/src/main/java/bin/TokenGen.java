...
/** <b>TokenGen使用方法:</b><br>
 *  <p>1、首先在调用generateToken();</p>
 *  <p>2、再将token值保存到request中XXXX;</p>
 *  <p>3、再在jsp的form中加个标签 < input type="hidden" name="XXXX" value="${requestScope.XXXX}" />
 *  <p>4、在该form的目的Servlet中获取实例,调用其 checkValue() 判断是否重复提交，调用checkValue()会删除session中的token</p>
 * */
